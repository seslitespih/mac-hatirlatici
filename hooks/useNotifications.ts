...
import { getMatchesHybrid } from '../services/matchService';
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (granted && enabled) {
        ...
        const matches = await getMatchesHybrid(countryCode);
  ...
  async function toggleNotifications() {
    ...
    if (newValue && permissionGranted) {
      if (matchesRef.current.length === 0) {
        const countryCode = (await getCountry()) ?? 'TR';
        matchesRef.current = await getMatchesHybrid(countryCode);
      }
